...
typeStat(set(X,Expr),Env,void) :- memT(X,Env,T), typeExpr(Expr,Env,T).
typeStat(ifst(Cond,prog(Bk1),prog(Bk2)),Env,void) :-  typeExpr(Cond,Env,bool), typeCmds(Bk1,Env,void),typeCmds(Bk2,Env,void).
typeStat(while(Cond,prog(Bk)),Env,void) :- typeExpr(Cond,Env,bool), typeCmds(Bk,Env,void).
typeStat(call(X,Es),Env,void) :- checkTypes(Es,Env,Targs), memT(X,Env,arrow(Targs,void)).

typeDec(var(X,T),Env,[(X,T)|Env]).
typeDec(proc(X,Args,prog(Bk)),Env,[(X,arrow(Targs,void))|Env]) :- concat(Args,Env,Res), tronc(Targs,Args), typeCmds(Bk,Res,void).
typeDec(procRec(X,Args,prog(Bk)),Env,[(X,arrow(Targs,void))|Env]) :- concat(Args,Env,Res), tronc(Targs,Args), typeCmds(Bk,[(X,arrow(Targs,void))|Res],void).
